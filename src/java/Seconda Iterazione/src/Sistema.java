...
public class Sistema {
    ...
    public void load_disponibilita1(int aulaId, String data) throws SQLException {
        ...
        while (rs.next()) {
            Disponibilita1 d1 = new Disponibilita1(rs.getInt("id_postazione"), rs.getInt("fktavolo"),
                    rs.getInt("id_turno"));
    ...
    public void load_disponibilita_multipla(int aulaId, String data, int n_studenti) throws SQLException {
        ...
        disponibilita.clear();
        while (rs.next()) {
            Disponibilita1 d1 = new Disponibilita1(rs.getInt("postazione_id"), rs.getInt("fktavolo"),
                    rs.getInt("turno_id"));
            disponibilita.add(d1);
        }
    }
    ...
    public void terminaInserimentoAula(DatabaseConnection dbConnection) {
        ...
        try (PreparedStatement stmt = dbConnection.conn.prepareStatement(query)) {
            ...
            Aula aula = this.listaAule.stream().filter(a -> a.getId() == tavoloCorrente.getCodiceaula()).findFirst()
                    .orElse(null);
            if (aula == null) {
    ...
    public void terminaInserimentoTavolo(DatabaseConnection dbConnection) {
        ...
        try (PreparedStatement stmt = dbConnection.conn.prepareStatement(query)) {
            ...
            String query2 = null;
            query2 = "UPDATE aula1 SET numerotavoli = numerotavoli + 1 WHERE id = "
                    + this.tavoloCorrente.getCodiceaula();
            PreparedStatement stmt2 = dbConnection.conn.prepareStatement(query2);
            ...
            Aula aula = this.listaAule.stream().filter(a -> a.getId() == tavoloCorrente.getCodiceaula()).findFirst()
                    .orElse(null);
            if (aula != null) {
                Tavolo tavolo = aula.getLista_tavoli().stream()
                        .filter(tavolo1 -> tavolo1.getId() == this.tavoloCorrente.getId()).findFirst().orElse(null);
                if (tavolo == null) {
                    for (int i = 0; i < this.tavoloCorrente.getN_postazioni(); i++) {
                        Postazione p = new Postazione(i, this.tavoloCorrente.getId(),
                                this.tavoloCorrente.getCodiceaula());
    ...
    public void checkStudentiExist(LinkedList<Integer> studenti) {
    ...
    // public void inserisciDatiPrenotazioneMultipla(int id_aula, String data)
    // throws SQLException {
    ...
    // System.out.println("Posto -> " + d1.id_postazione + ", Tavolo " +
    // d1.id_tavolo + ", Turno " + d1.turno);
    // }
    ...
    // System.out.println("Selezione la turno, digita 1 per la mattina, 2 per il
    // pomeriggio, 3 per la sera");
    // int fascia_scelta = scanner.nextInt();

    // Random rand = new Random();
    // String stato = "CONFERMATO";
    // prenotazioniCorrenti = new LinkedList<Prenotazione>();
    // for (Integer studente : studenti) {
    // int codPrenotazione = rand.nextInt(10000000);
    // this.prenotazioneCorrente = new Prenotazione(studente, codPrenotazione,
    // stringToDate(data), 1, id_aula,
    // tavolo_scelto, postazioniScelte.get(studenti.indexOf(studente)),
    // fascia_scelta, stato);
    ...
    // public void terminaPrenotazioneMultipla(DatabaseConnection dbConnection) {
    // String query;
    // query = "INSERT INTO `prenotazione2` (`id`, `fkstudente`, `fkpostazione`,
    // `fktavolo`, `fkaula`, `data`, `fkturno`, `fkmateria`, `stato`) VALUES (?, ?,
    // ?, ?, ?, ?, ?, ?, ?)";
    ...
    // prenotazioni.add(p);
    // System.out.println("Prenotazione inserita con successo! (" + rowsAffected + "
    // riga/i inserita/e)");

    // } catch (SQLException e) {
    // if (e.getErrorCode() == 1062) { // Codice di errore MySQL per violazione
    // UNIQUE KEY
    ...
    public void inserisciDatiPrenotazione(LinkedList<Integer> studenti2, int id_aula, String data) throws SQLException {
        ...
        Random rand = new Random();
        int codPrenotazione = rand.nextInt(10000000);
        this.prenotazioneCorrente = new Prenotazione(studenti2, codPrenotazione, stringToDate(data), 1,
                id_aula, tavolo_scelto, postazione_scelta, fascia_scelta, "CONFERMATO");
